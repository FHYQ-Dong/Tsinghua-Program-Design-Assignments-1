...
#define rightrotate(w, n) ((w >> n) | (w) << (32-(n)))
    ...
    #define copy_uint32(p, val) *((uint32_t *)p) = _byteswap_ulong((val))//_byteswap_ulong
    #else
    #error "Unsupported compiler! GCC or MSVC is required!"
    #endif
...
void sha256(const unsigned char *data, size_t len, unsigned char *out) {
    ...
    unsigned char *buf = (unsigned char *)malloc(new_len * sizeof(unsigned char));
    if (buf == NULL) {
